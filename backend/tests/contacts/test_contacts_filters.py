...
class ContactsFiltersTests(ContactsTestCase):
    ...
    async def test_list_contacts_blank_sort_falls_back_to_default(self, mock_list):
        # An explicit empty sort= behaves like an omitted param, not a 422.
        mock_list.return_value = ([], 0)
        app = self._override_current_user()
        try:
            resp = await self.client.get("/api/v1/contacts?sort=")
        finally:
            self._clear_override(app)
    ...
    @patch("app.modules.contacts.repository.ContactRepository.list", new_callable=AsyncMock)
    async def test_list_contacts_blank_q_is_treated_as_omitted(self, mock_list):
        mock_list.return_value = ([], 0)
        app = self._override_current_user()
        try:
            resp = await self.client.get("/api/v1/contacts?q=%20%20")
        finally:
            self._clear_override(app)
        self.assertEqual(resp.status_code, 200)
    ...
    @patch("app.modules.contacts.repository.ContactRepository.list", new_callable=AsyncMock)
    async def test_list_contacts_trims_q_whitespace(self, mock_list):
        mock_list.return_value = ([], 0)
        app = self._override_current_user()
        try:
            resp = await self.client.get("/api/v1/contacts?q=%20ada%20")
        finally:
            self._clear_override(app)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(mock_list.call_args.kwargs["q"], "ada")
